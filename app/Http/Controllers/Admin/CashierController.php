...
{        
       $caixaToday = $this->getCaixa()[0]->total - $this->getNegative()[0]->total ;
        ...
        // dd( Cashier::pluck('id'));
          ...
          $query = $this->model->where("caixa_status_id",'=',3)->orderBy('cashier.created_at', 'desc')
        ...
        compact(
            'caixaToday','categories','products','day','prodK','caixaAberto','clients','paginator'
        ));
    }
    ...
    public function list(){
        ...
        $query =  Cashier::select(
            \DB::raw(
               "SUM(cashier.valor) as total,
               COUNT(cashier.id) as operacoes,
               cashier.id, 
               cashier.caixa_status_id, 
               cashier.valor,
               cashier.acao,
               cashier.tipo,
               cashier.obs,
        ...
        ->groupBy(\DB::raw("cashier.caixa_status_id")) 
        ->join('caixa_status', 'cashier.caixa_status_id', '=', 'caixa_status.id')->get();
     ...
     public function show($id){
        ...
        $query = $this->model->where("caixa_status_id",'=',$id)->orderBy('cashier.created_at', 'desc')
        ->join('caixa_status', 'cashier.caixa_status_id', '=', 'caixa_status.id');
        ...
        $viewData = compact(['clients', 'paginator']);
        return view('admin.cashier.details', $viewData);
