...
if($request->ajax()){
    ...
    $mytime = \Carbon\Carbon::now(); 
        ...
        $semanaDay = $dias[$mytime->toArray()['dayOfWeek']];
         
        $configsL = DeliveryConfig::select('*')
        ->where('config_date', '=', $semanaDay	)
        ->where('config_loja_id', '=', 3	)
        ->where('config_status', '=', '1'	)->first()
    ;
       
        //dd(date_format(date('h:i:s',strtotime($configsL->config_time)), "H:i"));
        $shopName = Loja::find(3); 
        $data = $request->all();
        ...
        $products = Product::select(
            ...
            ->where('product.product_lojas_id', 'like', '%' . '3' . '%')
        ...
        $configs = DeliveryConfig::select('*')
            // ->where('config_loja_id', '=', $data['id-loja'])
            ->get();
        ...
        $banner = PromotionsBanners::select('*')
            ...
            ->get();
             ...
             $bannerCarrinho = PromotionsBanners::select('*')
            ->with('getProduct')
            ->where('promo_banner_day', '=', $day)
            ->where('promo_status', '=', '1')
...
//End live search
    ...
    public function prodCategorie(Request $request)
    {
        ...
        return view('delivery.index-new', compact('products', 'categories', 'banner', 'variations', 'catId', 'promoProducts', 'bannerCarrinho' ));
    }
    ...
    public function deliveryProductsTp(Request $request)
    {  
        $mytime = \Carbon\Carbon::now(); 
    ...
    ;
        ...
        // return view('delivery.index', compact('tax', 'shopName', 'products', 'loja', 'configs', 'categories', 'banner', 'variations', 'promoProducts'));
        return view('delivery.index-new', compact('shopName','products', 'categories', 'banner', 'variations', 'promoProducts' , 'bannerCarrinho','configsL'));
    } 
    public function deliveryProductsSV(Request $request)
    {  
        $mytime = \Carbon\Carbon::now(); 
        ...
        $configsL = DeliveryConfig::select('*')
        ->where('config_date', '=', $semanaDay	)
        ->where('config_loja_id', '=', 4	)
        ->where('config_status', '=', '1'	)->first()
    ; 
        $shopName = Loja::find(4); 
        $data = $request->all();  
        ...
        $products = Product::select(
            ...
            ->where('product.product_lojas_id', 'like', '%' . '4' . '%')
    ...
    public function deliveryProductsBQ(Request $request)
    {  
    ...
    ;
        ...
        $products = Product::select(
            ...
            ->where('product.product_lojas_id', 'like', '%' . '1' . '%')
    ...
    public function idEntrega()
    {
        ...
        if (!empty($data) && isset($data)) {
            return view('delivery.identificacao-entrega')->with('data');
        }
