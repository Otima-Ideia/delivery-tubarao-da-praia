				...
				for(let c of complementos){
					if(c.comp == i.comp){
						listaCmp += `
							<li class="clearfix">
								<div>+ ${c.nome}</div>
								<div class="cmvl-${i.comp}">${numberToReal(c.preco)}</div>
							</li>
						`;
					}
				}
	...
	for(let t of obj) {   
		...
		for(let c of comp){   
			if(t.comp == c.comp){
				...
				jQuery('.cmvl-' + c.comp).html(					numberToReal(parseFloat(t.qtd * c.preco))
				);
			}
		}	
	}
...
function removeCompList(comp,compLista){
	for(i = 0; i < compLista.length; i++) { 
		if(compLista[i].comp == comp){
			var listAux    = compLista.slice(0, i);
			var listAuxEnd = compLista.slice(i + 1)
			return complementos = listAux.concat(listAuxEnd); 
		}
	}
	return false;
}
...
function complementosModal(id,qtd,nome,preco,checkbox,obj){
	...
	`;
	jQuery('.escolher .conteudo').html(conteudo);
}
