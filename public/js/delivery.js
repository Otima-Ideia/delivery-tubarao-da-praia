...
function getTotal() {
  if (
    localStorage.getItem("prods") != undefined &&
    Object.keys(localStorage.getItem("prods")).length !== 0
  ) {
    var obj = JSON.parse(Base64Decode(localStorage.getItem("prods")));
  }
  if (
    localStorage.getItem("complementos") != undefined &&
    Object.keys(localStorage.getItem("complementos")).length !== 0
  ) {
    ...
    for (let t of obj) {
      ...
      for (let c of comp) {
        if (t.comp == c.comp) {
          ...
          jQuery(".cmvl-" + c.comp + "-" + c.id_complemento).html(
            numberToReal(parseFloat(t.qtd * c.preco))
          );
        }
      }
    }
...
function getPayments(methods) {
  html =
    '<label for="Forma de pagamento">Forma de Pagamento:<select class="style-input" name="pagamento" id="forma-pagamento"></label>';
...
function pesquisaScrollBtn(texto) {
  ...
  if (scroll != undefined) {
    document.location =
      "http://pedidos.tubaraodapraia.com.br/delivery/search=" + parseURL(texto);
    // jQuery('html,body').animate({
    // 	scrollTop: scroll.top - 140
    // }, 100);
...
function inicializacao(ID, tx) {
  ...
  if (!complementos) {
    ...
    localStorage.setItem(
      "complementos",
      Base64Encode(JSON.stringify(complementos))
    );
  }
  ...
  jQuery(".item-p").each(function () {
    ...
    jQuery(".pesquisa-resultado").append(
 "<li" + " onclick=" + "'onClickFunctionAlter()'" +">" + replaceAll(produto, "-", " ")  + "</li>"
    );
    jQuery(".pesquisa-resultado-mobile").append(
      "<li>" + replaceAll(produto, "-", " ") + "onclick=" + "'onClickFunctionAlter()'" + "</li>"
    );
  });
...
function agendamento() {
  ...
  if (time) {
    ...
    if (agendamento >= abertura && agendamento <= fechamento) {
      ...
      jQuery("#agendar-hora").after(
        "<p class='erromsg'>Não realizamos entregas nesse horário</p>"
      );
    }
  } else {
    jQuery("#agendar-hora").after(
      "<p class='erromsg'>Você não preencheu o horário</p>"
    );
  }
}
...
function adicionarProdutoVariavel(
  id,
  idVar,
  idEntrega,
  qtd,
  ...
  if (comps != null) {
    for (let c of comps) {
      for (let t of obj) {
        if (c == t.id) {
          checkbox += `
    			<label class="checkbox-style" price="${t.price_complement}" name="${
            t.name_complement
          }" id="${t.id}" product="${id}">
        			<input class="checkbox-item" type="checkbox"/>
					<span class="name">${t.name_complement}</span>
					<span price="${t.price_complement}" class="price">${numberToReal(
            t.price_complement
          )}</span>
				</label>
			`;
	...
	${nome}
		<span class="compl-price-prod" price="${preco}" style="float: right;">${numberToReal(
      preco
    )}</span>
		</p>
...
`;
    ...
    if ($(".add-comp").length < 1) {
      $(".escolher .conteudo").append(
        '<input style="background-image: url(img/continuar.png);" class="add-comp" value="Continuar" type="button"></input>'
      );
    }
...
function adicionarComplemento(produtosLista, complementosLista) {
  ...
  localStorage.setItem(
    "complementos",
    Base64Encode(JSON.stringify(complementosLista))
  );
...
function getComboModal(id, nomeProduto, preco) {
  ...
  for (let cv of comboVariations) {
    ...
    if (cv.cat_id != lsCat && cv.prod_id == id) {
      if (
        cv.get_category.name_category == "Pizzas Grandes" &&
        cv.num_esc == "2"
      )
            ...
            jQuery(".escolher__conteudo label").each(function () {
              if (
                jQuery(this).children("input[type=checkbox]").prop("checked")
                  ...
                  preco += parseInt(
                    jQuery(this).children(".price").attr("price")
                  );
              }
            });
    ...
    if (
      cv.cat_id == lsCat &&
      idsPassados.indexOf(cv.id) == -1 &&
      cv.prod_id == id
    ) {
      checkbox += `
				<label class="checkbox-style" price="${cv.price_product}" name="${
        cv.variation_name
      }" id="${cv.variation_name + "-" + cv.cat_id}" product="${id}">
					<input onchange='checkLimit(${cv.num_esc}, "${cv.cat_id}");' price="${
        cv.price_product
      }" class="checkbox-combo-item ${cv.variation_name}" num_esc="${
        cv.num_esc
      }" name="${cv.variation_name}" cat_id="${
        cv.cat_id
      }" type="checkbox" value="${cv.id}"/>
					<span class="name">${cv.name_product}</span>
					<span price="${cv.price_product}" class="price">${numberToReal(
        cv.price_product
      )}</span>
				</label>
				`;
