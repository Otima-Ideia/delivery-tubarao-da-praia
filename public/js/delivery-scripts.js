...
$(document.body).on("click", ".checkbox-combo-item", function (event) {
  ...
  if (
    $('.checkbox-combo-item[cat-id="' + cat_id + '"]:checked').length > num_esc
  ) {
...
$(document.body).on("click", "#add-combo", function (event) {
  ...
  for (let c of cats) {
    comboSelected[c] = $(
      '.checkbox-combo-item[cat_id="' + c + '"]:checked'
    ).length;
  }
  ...
  addComboProduct(
    JSON.parse(Base64Decode(localStorage.getItem("prods"))),
    ids,
    $(this).attr("prod_id"),
    $(this).attr("prod_name"),
    $(".compl-price-prod").text()
  );
});
...
$(document).ready(function () {
  ...
  jQuery(document.body).on(
    "click",
    ".pesquisa-resultado li, .pesquisa-resultado-mobile li",
    function () {
...
$(document.body).on("click", ".add-prod", function () {
  ...
  if (
    localStorage.getItem("prods") != null &&
...
(function () {
  ...
  var val =
    window_height -
    (header_height + produtos_nav + titulo + valores + valores2 + comprar);
...
if ($(window).innerWidth() > 1121) {
  $(window).scroll(function () {
    var header_height =
      $("header").height() +
      $(".produtos-banner").height() +
      $(".produtos__banner").height();
